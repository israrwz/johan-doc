...
static ngx_uint_t   ngx_show_help;	//定义几个静态全局变量 通过ngx_get_options设置值
...
int ngx_cdecl
main(int argc, char *const *argv)//------------入口点
{
    ngx_int_t         i;
    ngx_log_t        *log;	// 日志对象
    ...
    if (ngx_strerror_init() != NGX_OK) {	//error输出设备初始化
        return 1;
    }
    ...
    if (ngx_show_version) {	// 显示版本信息	---前提条件
        ngx_log_stderr(0, "nginx version: " NGINX_VER);

        if (ngx_show_help) {	// 显示命令帮助列表
            ngx_log_stderr(0,
...
#if (NGX_FREEBSD)	// 编译调试配置
    ngx_debug_init();
#endif

    /* TODO */ ngx_max_sockets = -1;

    ngx_time_init();	//服务器时间初始化
    ...
    log = ngx_log_init(ngx_prefix);	// 服务器日志初始化
    if (log == NULL) {
        return 1;
    }
    ...
    ngx_memzero(&init_cycle, sizeof(ngx_cycle_t));	//初始化init_cycle 分配内存
    init_cycle.log = log;
    ngx_cycle = &init_cycle;	//全局对象 ngx_cycle

    init_cycle.pool = ngx_create_pool(1024, log);	// 设置内存池
    if (init_cycle.pool == NULL) {
        return 1;
    }

    if (ngx_save_argv(&init_cycle, argc, argv) != NGX_OK) {//保存参数
        return 1;
    }

    if (ngx_process_options(&init_cycle) != NGX_OK) {//设置init_cycle的一些字段
        return 1;
    }

    if (ngx_os_init(log) != NGX_OK) {//配置一些和操作系统相关的全局变量
        return 1;
    }
    ...
    if (ngx_crc32_table_init() != NGX_OK) { // 调用初始化函数
        return 1;
    }

    if (ngx_add_inherited_sockets(&init_cycle) != NGX_OK) {//初始化 sockets
        return 1;
    }

    ngx_max_module = 0;
    for (i = 0; ngx_modules[i]; i++) {	//给所有模块编号
        ngx_modules[i]->index = ngx_max_module++;
    }

    cycle = ngx_init_cycle(&init_cycle);	// 主要模块初始化
    if (cycle == NULL) {
        if (ngx_test_config) {
    ...
    if (ngx_signal) {	//信号量的相关处理
        return ngx_signal_process(cycle, ngx_signal); //单进程
    }
...
static ngx_int_t
ngx_get_options(int argc, char *const *argv)	// 解析参数
{
    u_char     *p;
    ngx_int_t   i;

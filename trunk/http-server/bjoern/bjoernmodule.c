...
listen(PyObject* self, PyObject* args)
{
  ...
  if(wsgi_app) {
    PyErr_SetString(	//----调用python抛出异常方法
      PyExc_RuntimeError,
  ...
  char * msg = "host:%s  port:";

  PyRun_SimpleString("import os\n"	//运行一段代码
  "print os\n");

  if(!server_init(host, port)) {	//初始化服务器 创建套接字
    PyErr_Format(
      PyExc_RuntimeError,
...
//  PyRun_SimpleString("print wsgi_app\n");	//运行一段代码
  ...
  Py_RETURN_NONE;
	...
	//----------------------------------------------------------------------------------------------------c模块函数名称映射表
static PyMethodDef Bjoern_FunctionTable[] = {	// python 扩展模块funtion定义
  {"run", run, METH_VARARGS, run_doc},	//bjoern.run(wsgi_application, host, port)
  {"listen", listen, METH_VARARGS, listen_doc}, //bjoern.listen(wsgi_application, host, port)
  {NULL, NULL, 0, NULL}
};

PyMODINIT_FUNC initbjoern()	//c模块的初始化函数
{
  _init_common();
  _init_filewrapper();

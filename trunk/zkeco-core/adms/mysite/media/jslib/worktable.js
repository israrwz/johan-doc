...
function init_commom_search(surl,dbapp_url){
		...
		var s_900=["employee_num","identitycard","device"];
...
function process_scroll(){
	...
	if($.browser.msie){
		for(var i=0;i<bdiv.length;i++){
			if(bdiv[i].scrollWidth>bdiv[i].clientWidth){
...
function init_specday(surl,dbapp_url){
	...
	var opt=getPubOpt("att","EmpSpecDay",["state__in=1,4"])
	new Mgrid(opt);
}
//加班单
function init_overtime(surl,dbapp_url){
	...
	var opt=getPubOpt("att","OverTime",["auditstates__in=1,4"])
	new Mgrid(opt);
}

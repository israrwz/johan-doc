...
function createTZTable(tzData, dayLabelFun, dayCount,sDate)	//-------------画班次表格
{
    var N=tzData.N[0];	// [40, "\u673a\u7535\u73ed", "2011-07-14", "2011-07-14", 2, 1]
    tzData=tzData.data;
    var pos,tdays
	
	//计算当前日期在班次模板中的起始位置
    post=0
    if(N[5]==1)//周		//---N[5] 周期单位	 N[4]周期个数
    {
        tdays=N[4]*7 	//----班次总天数
        pos=sDate.getDay() 
    }
	...
	while(d<dayCount)
	{
		...
		else
		{
			...
			for(var si=0;si<sub.length;si++)
			{
				...
				html+='<div alt0="'+tz['StartTime']+'" alt1="'+tz['EndTime']+'" alt2="'+getTZDateAlt(d,sDate)+'" alt4="'+tz['SchClassID']+'" style="text-align: center; width: '+normalP(time2-time1)+'px;'+
...
$(function(){
    ...
    $.ajax({
        url:"../../../att/choice_widget_for_select_emp/?multiple=T&name=UserID",
        type:"POST",
        dataType:"html",

...
def render_emp_widget(name, data, multiple, popup, filter):
    ...
    try:
        ...
        url=data and ("%s%s/Employee/?id__exact=%s"%(dbapp_url,Employee._meta.app_label,data)) or ("%s%s/Employee/?%s"%(dbapp_url, Employee._meta.app_label, filter))
    ...
    def value_from_datadict(self, data, files, name):
        ...
        if isinstance(data, (MultiValueDict, MergeDict)):
            ...
            if dept_all:#选择部门下所有人员
                ...
                return [e.pk for e in Employee.objects.filter(DeptID__in = dept_id)]
            else:

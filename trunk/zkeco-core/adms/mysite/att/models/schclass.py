...
class SchClass(CachingModel):
    ...
    StartRestTime = models.TimeField(_(u'段中休息开始时间'),null=True, blank=True,editable=True)             #暂时不编辑,
    EndRestTime = models.TimeField(_(u'段中休息结束时间'),null=True, blank=True,editable=True)                  #暂时不编辑
    ...
    shiftworktime = models.IntegerField(_(u'工作时间(分钟)'),null=False,blank=False,editable=True,default=480,help_text=_(u"0:表示加班时间段，计算的时候将会直接计算成平时加班。"))
    IsOverTime=models.SmallIntegerField(_(u'延时是否计加班'),default=1,choices=BOOLEANS)
    OverTime=models.IntegerField(_(u'该时段内包含固定加班时间(分钟)'),null=True,blank=True, default=0)
...
#        print et
    ...
    class Admin(CachingModel.Admin):
        ...
        list_display=('SchName','StartTime','EndTime','WorkDay','CheckIn','CheckOut','IsOverTime','OverTime')
        hide_fields=('SchclassID',)
        help_text=_(u'1、不能与已存在的时段有相同上班时间和下班时间<br>2、必须签到与签退，选取否时，系统在计算时会自动创建相应的随机签卡!')
        ...
        def initial_data(): #
                ...
                if SchClass.objects.count()==0:
                        SchClass(SchName=u"%s"%_(u"弹性时段"), 
                                ...
                                CheckInTime2 = datetime.datetime.strptime("23:00","%H:%M").time(),
                                CheckOutTime1 = datetime.datetime.strptime("01:00","%H:%M").time(),

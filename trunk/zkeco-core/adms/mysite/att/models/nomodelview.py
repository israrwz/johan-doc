...
@login_required
def att_abnormal_report(request):
    '''
    考勤异常表
    '''
    from mysite.att.att_calculate_utils import AttCalculateBase
#    dic = {'pin':u'人员编号','name':u'人员姓名','DeptName':u'所属部门','ClockInTime':u'上班时间','ClockOutTime':u'下班时间','StartTime':u'签到时间','EndTime':u'签退时间','AbnormalType':u'异常类型'}
    dic = (('name',u'人员姓名'),('pin',u'人员编号'),('DeptName',u'所属部门'),('ClockInTime',u'上班时间'),('ClockOutTime',u'下班时间'),('StartTime',u'签到时间'),('EndTime',u'签退时间'),('AbnormalType',u'异常类型'))
    ...
    sql = '''
        ...
        attshifts.ClockInTime,
        attshifts.ClockOutTime,
        attshifts.StartTime,
        attshifts.EndTime,
        case when attshifts.Late>1 AND attshifts.Early>1 then '迟到早退' when attshifts.Late>1 then '迟到' when attshifts.Early>1 then '早退' when attshifts.Absent>1 then '矿工' end AS AbnormalType
        FROM
        attshifts,userinfo,Departments
        where (attshifts.Late>1 or attshifts.Early>0 or attshifts.Absent>1) and attshifts.userid=userinfo.userid and Departments.deptID=userinfo.defaultdeptid
        order by departments.DeptID,userinfo.badgenumber
    '''
    cs=customSql(sql,False)
    desc=cs.description
    filedlist = [e[0] for e in desc]
    rows=cs.fetchall()
    for rs in rows:
        obj = calculate.NewItem()
        obj['pin'] = str(rs[filedlist.index('badgenumber')])
        obj['name'] = str(rs[filedlist.index('name')])
        obj['DeptName'] = str(rs[filedlist.index('DeptName')])
        obj['ClockInTime'] = str(rs[filedlist.index('ClockInTime')])
        obj['ClockOutTime'] = str(rs[filedlist.index('ClockOutTime')])
        obj['StartTime'] = str(rs[filedlist.index('StartTime')])
        obj['EndTime'] = str(rs[filedlist.index('EndTime')])
        obj['AbnormalType'] = rs[filedlist.index('AbnormalType')]
        calculate.AddItem(obj)
    return getJSResponse(smart_str(dumps(calculate.ResultDic(1))))

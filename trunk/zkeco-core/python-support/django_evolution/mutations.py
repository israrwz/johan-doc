...
class MockMeta(object):
    ...
    def __init__(self, proj_sig, app_name, model_name, model_sig):
        ...
        self.meta = {
            ...
            'has_auto_field': None,#j------------------- 外置
            'db_tablespace': None,
        }
        self.meta.update(model_sig['meta']) #j------------添加 meta 成员
        self._fields = SortedDict()  #j--------两个可排序字典成员
        self._many_to_many = SortedDict()
        self.abstract = False   #j----------------三个布尔成员
        self.managed = True
        self.proxy = False
        self._model_sig = model_sig #j-------------模型和proj 结构字典
        self._proj_sig = proj_sig

    def setup_fields(self, model, stub=False):
        for field_name, field_sig in self._model_sig['fields'].items():
            if not stub or field_sig.get('primary_key', False): #j-------------处理主键字段
                field_type = field_sig.pop('field_type') #j-----取出字段类型名
                ...
                if ManyToManyField == type(field):
                    ...
                    self._fields[field.name] = field    #j-----------------字段列表

                field.set_attributes_from_name(field_name)
                if field_sig.get('primary_key', False):
                    self.pk = field #j----------------设置主键
...
class MonoBaseMutation(BaseMutation):
    ...
    def __init__(self, model_name = None):
        ...
        self.model_name = model_name    #j---------------------定义成员model_name 默认为None
...
class DeleteField(MonoBaseMutation):
    ...
    def simulate(self, app_label, proj_sig, database=None):
        ...
        if model_sig['fields'][self.field_name].get('primary_key',False):   #j--------------主键不能被删除
...
#        issubclass(field_type, models.ManyToManyField)
        if issubclass(field_type, models.ManyToManyField):
            sql_statements = []
            act_table = field._get_m2m_db_table(model._meta)
            act = lambda:db.delete_table(act_table) 
            ac_doc = 'table:[%s] delete table [%s] ' %(act_table,act_table)
            do_act = mutateAction(act,ac_doc)
            sql_statements.append(do_act)
#            sql_statements = \
#                evolver.delete_table(field._get_m2m_db_table(model._meta))#j-------------------------------带改点-------------   删除表
        else:
#            sql_statements = evolver.delete_column(model, field)

            sql_statements = []
            act = lambda:db.delete_column(model._meta.db_table,field.column)
            ac_doc = 'table:[%s] delete column [%s] ' %(model._meta.db_table,field.column)
            do_act = mutateAction(act,ac_doc)
            sql_statements.append(do_act)
...
class AddField(MonoBaseMutation):
    def __init__(self, model_name, field_name, field_type,
                 initial=None, **kwargs):
        MonoBaseMutation.__init__(self, model_name)
        self.field_name = field_name    #j---------------------定义了三个成员变量
        self.field_type = field_type
        self.field_attrs = kwargs
        self.initial = initial

    def __str__(self):
        params = (self.model_name, self.field_name, self.field_type.__name__)   #j--------------- 模型、字段名、字段类型名等信息
        str_output = ["'%s', '%s', models.%s" % params]

        if self.initial is not None:    #j----------------------如果有初始化信息则显示出来
            str_output.append('initial=%s' % repr(self.initial))

        for key,value in self.field_attrs.items():  #j------------------------遍历字段的所有属性键值对
    ...
    def simulate(self, app_label, proj_sig, database=None):
        app_sig = proj_sig[app_label]
        model_sig = app_sig[self.model_name]    #j------------------model

        if self.field_name in model_sig['fields']:  #j--------------------判断是否有重名的字段存在
            raise SimulationFailure(
                "Model '%s.%s' already has a field named '%s'"
                % (app_label, self.model_name, self.field_name))

        if (self.field_type != models.ManyToManyField and   #j-----------------------判断是否没有任何"null"信息
        ...
        model_sig['fields'][self.field_name].update(self.field_attrs)   #j-----将新字段加入结构字符串字典中
...
#        return mutateAction()
    ...
    def add_column(self, app_label, proj_sig):
        app_sig = proj_sig[app_label]
        model_sig = app_sig[self.model_name]    #j---------得到模型结构字典
...
#        sql_statements.extend(evolver.create_index(model, field))
        ...
        sql_statements = []
        act = lambda:db.add_column(model._meta.db_table,field.column,field)
        ac_doc = 'table:[%s] add column [%s] with %s' %(model._meta.db_table,field.column,str(field.__class__))
        do_act = mutateAction(act,ac_doc)
        sql_statements.append(do_act)
        # Create SQL index if necessary
        act_index = lambda:db.create_index(model._meta.db_table,[field.column])
        ac_doc_index = 'table:[%s] create_index on [%s]' %(model._meta.db_table,field.column)
        sql_statements.append(mutateAction(act_index,ac_doc_index))
...
class ChangeField(MonoBaseMutation):
    def __init__(self, model_name, field_name,current_field_sig, initial=None, **kwargs):
        MonoBaseMutation.__init__(self, model_name)
        self.field_name = field_name
        
        self.field_type = current_field_sig['field_type']
        for key in current_field_sig.keys():   #j-----------add
            if key!='field_type':
                kwargs[key] = current_field_sig[key]
#        self.current_field_sig = current_field_sig #j-----------add
#        del self.current_field_sig['field_type']
        self.field_attrs = kwargs
        self.initial = initial
    ...
    def mutate(self, app_label, proj_sig, database=None):
        ...
        field = create_field(proj_sig, self.field_name, self.field_type,
                             self.field_attrs, model)   #j----------------add

        sql_statements = []

        act = lambda:db.alter_column(model._meta.db_table,field.column,field)
        ac_doc = 'table:[%s] alter column [%s] with %s' %(model._meta.db_table,field.column,str(field.__class__))
        do_act = mutateAction(act,ac_doc)
        sql_statements.append(do_act)
...
class mutateAction(object):
    def __init__(self,func=(lambda:None),disc=None):
        self.doAction = func

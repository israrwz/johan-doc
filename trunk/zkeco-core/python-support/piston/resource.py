    ...
    def __call__(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not isinstance(stream, HttpResponse):
                emitter, ct = Emitter.get(em_format) #ct may be changed
                resp = HttpResponse(stream, mimetype=ct)
                accept_charset=request.META.get('HTTP_ACCEPT_CHARSET', "gb18030")
                if "gb18030" in accept_charset: 
                    accept_charset="gb18030"
                else:
                    accept_charset="utf-8"
                resp['Content-Disposition'] = (u'attachment; filename=%s.%s'%(hasattr(srl, "title") and srl.title or "temp", em_format)).encode(accept_charset)
            else:
                resp = stream

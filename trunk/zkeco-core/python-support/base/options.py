...
class AppOption(CachingModel):
        ...
        class Admin(CachingModel.Admin):
                ...
                def initial_data(self):
                    ...
                    att = "mysite.att" in settings.INSTALLED_APPS
                    if att and not AppOption.objects.filter(optname='msg_scanner'):
...
class AppOptionClass(object):
        '''
        定义 appOptions 类items对象  将数据库数据映射成可读写的对象
        '''
...
class OptionClass(object):
        ...
        def add_option(self, item, default, verbose_name, help_text, catalog, visible=True):
                ...
                try:
                        ...
                        opt = Option(app_label=items[0], name=items[1], default=default, verbose_name=verbose_name, help_text=help_text , catalog=catalog, visible=visible)
                        opt.save()
                return opt

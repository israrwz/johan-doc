...
class adminForm(forms.Form):
        ...
        def save(self):
                ...
                try: 
                        ...
                        for t in depts:
                                d=Department.objects.get(id=t.pk)
                                DeptAdmin(user=u, dept=d).save()
                        
                        if self.areaadmins:
                                self.areaadmins.delete()
                        areas = self.cleaned_data['AuthedArea']
                        #if not areas:return u
                        for t in areas:
                                d=Area.objects.get(id=t.pk)
...
def retUserForm(request, f, isAdd=False):
        ...
        kargs["app_menu"]=hasattr(User.Admin,"app_menu") and User.Admin.app_menu or  User._meta.app_label
        ...
        if fp_user:
            ...
            if fingger10 and fptemplate10:
                ...
                for i in range(len(fingger10)):
                    t10 = OperatorTemplate.objects.filter(user=fp_user[0], finger_id__exact=fingger10[i], fpversion="10")
                    if not t10:

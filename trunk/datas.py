...
def ClearTableData(TableName, Condition=''):
    if Condition!='':
        sqlstr='delete from %s where (%s)'%(TableName,Condition)
    else:
...
def CalcReportItem_sf(request,deptids,userids,d1,d2,reportType=0,totalall=False):
    '''
    考勤及年休假统计表
    '''
    from mysite.att.models import AttException,attShifts,CheckExact
    global AbnomiteRptItems
    global ExceptionIDs
    global AttRule
    global schClasses
    AttRule=LoadAttRule()   #---考勤参数规则
    AbnomiteRptItems=GetLeaveClasses()  #---计算项目
    GetRptIndex(AbnomiteRptItems) #---得到有排序编号的 LeaveId
    
    if len(userids)>0 and userids!='null':
        ids=userids.split(',')
        ot=['DeptID','PIN']
        ids=Employee.objects.filter(id__in=ids,OffDuty__lt=1).values_list('id', flat=True).order_by(*ot)
    elif len(deptids)>0:
        deptIDS=deptids.split(',')
        deptids=deptIDS
        print "deptIDS:%s"%deptIDS
    ...
    try:
        ...
        if not totalall:
            limit= int(request.POST.get('l', settings.PAGE_LIMIT))  #导出时使用
            from mysite.att.att_calculate_utils import paging
            paging_info = paging(len(ids),offset,limit)
            ids = ids[paging_info[0]:paging_info[1]]
            Result.update(paging_info[2])          
        for u in ids:
            uids.append(u)
            
        if reportType==0:  #---根据不同的统计类型 构造不同的字段
            r,Fields,Capt=ConstructFields_sf()
        Result['fieldnames']=Fields
        Result['fieldcaptions']=Capt
        Result['datas']=r        
        for uid in uids:#---------------------------------循环统计每个人员
            uid=int(uid)
            attExcept=AttException.objects.filter(UserID=uid,AttDate__gte=d1,AttDate__lte=d2).order_by('UserID')
            checkexact=CheckExact.objects.filter(UserID=uid,audit_status=2,CHECKTIME__gte=d1,CHECKTIME__lte=d2).order_by('UserID').count()
            if settings.DATABASES["default"]["ENGINE"] == "django.db.backends.postgresql_psycopg2":
                sql="""select s.userid as userid,u.badgenumber as pin,u.name as name,u."SSN" as ssn,s."SchId" as schid,s."AttDate" as attdate,d."DeptName" as deptname,s."ClockInTime" as clockintime,s."ClockOutTime" as clockouttime,
                        s."StartTime" as starttime,s."EndTime" as endtime,s."WorkDay" as workday,s."RealWorkDay" as realworkday,s."NoIn" as noin,s."NoOut" as noout,
                        s."Early" as early,s."Late" as late,s."Absent" as absent,s."AbsentR" as absentr,s."OverTime" as overtime,s."ExceptionID" as exceptionid,s."MustIn" as mustin,s."MustOut" as mustout,  
            ...
            if not rows:
                ...
                rmdAttday['badgenumber']=emp.PIN
                rmdAttday['username']=emp.EName
            rmdAttday['checkexact'] =  str(checkexact)   
            for t in rows:
                row+=1
                if rmdAttday['userid']==-1:
                    rmdAttday['userid']=t[fldNames['userid']]
                ...
                if reportType==0:
                    rmdAttday['duty']=SaveValue(rmdAttday['duty'],t[fldNames['workday']])
                    rmdAttday['realduty']=SaveValue(rmdAttday['realduty'],t[fldNames['realworkday']])
                    rmdAttday['late']=SaveValue_n(rmdAttday['late'],t[fldNames['late']])
                    rmdAttday['early']=SaveValue_n(rmdAttday['early'],t[fldNames['early']])
                    if t[fldNames['absent']]>0:
                        try:
                            rmdAttday['absent']=SaveValue(rmdAttday['absent'],t[fldNames['absent']])
                        except:
...
def ConstructFields_sf():
    """为汇总报表构造字段  """
    global LClasses1

    r,FieldNames,FieldCaption=ConstructBaseFields_sf()
    LClasses1=GetLeaveClasses(1)
    for t in LClasses1:
        fName='Leave_'+str(t['LeaveId'])
        FieldNames.append(fName)

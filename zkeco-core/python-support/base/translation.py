        ...
        def get_field_display(self, model, field_name, value, language=None):
                ...
                if value: #该字段不是空值
                        try:
                                ...
                                if value2==None:
                                        try:
                                                value2=self.objects.get(content_type=ct, property=field_name, language=language, value=value).display
                                        except ObjectDoesNotExist, e:
                                                if "_" in language:
                                                        value2=self.objects.get(content_type=ct, property=field_name, language=language.split("_")[0], value=value).display
                                                else:
                                                        raise e
                                        if value2!=None:

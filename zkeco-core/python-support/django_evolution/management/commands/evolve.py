...
class Command(BaseCommand):
    ...
    def evolve(self, *app_labels, **options):   #j--------------------子程序参数解析
        verbosity = int(options['verbosity'])   
        interactive = options['interactive']    #j----------------------是否显示确认提示
        execute = options['execute']
        compile_sql = options['compile_sql']
        hint = options['hint']
        purge = options['purge']    #j----------------------删除已被去除的app相关的表
                ...
                if mutations:
                    ...
                    for mutation in mutations:
                        ...
                        if compile_sql or execute:
                            tar_johan = mutation.mutate(app_label, database_sig,database)
                            do_actions.extend(tar_johan)
#                            app_sql.extend(
#                                mutation.mutate(app_label, database_sig,
#                                                database))      #j-----------------------------------------切入点
                            app_sql.extend(tar_johan)
#                            mutation.mutate(app_label, database_sig,database)[0].doAction()
#                            mutation.mutate(app_label, database_sig,database)[1].doAction()

...
def control_services(YesNoClose, local_db=True):
    ...
    if YesNoClose:
        ...
        if "mysite.att" in settings.INSTALLED_APPS:
            ...
            os.system("net stop ZKECOZksaasAdmsService")
            os.system("net stop ZKECOAutoCalculateService")
    else:
        os.system("net start ZKECOWEBService")
        os.system("net start ZKECOBackupDB")
        os.system("net start memcached")
        if local_db:
            os.system("net start ZKECOPGSQL")
        if "mysite.iaccess" in settings.INSTALLED_APPS:
        ...
        if "mysite.att" in settings.INSTALLED_APPS:
            ...
            os.system("net start ZKECOZksaasAdmsService")
            os.system("net start ZKECOAutoCalculateService")
...
class DialogDb(wx.Dialog):
    def _init_ctrls(self, prnt):
        ...
        self.txt_dbtype = wx.ComboBox(choices=['mysql','oracle10g','sqlserver2005','postgresql'],id=wxID_DIALOG1TXT_DBTYPE,
    ...
    def Ontxt_dbtypeKillFocus(self, event):
        if self.txt_dbtype.Value=="sqlserver2005":
            ...
            elif self.txt_dbtype.Value=="postgresql":
                self.txt_port.Value="17771"
...
class RegisterLicence(wx.Dialog):
    def _init_ctrls(self, prnt):
        # generated method, don't edit
        wx.Dialog.__init__(self, id=wxID_DIALOG1, name='', parent=prnt,
              pos=wx.Point(462, 263), size=wx.Size(470, 275),
        ...
        self.frame_customer = wx.StaticBox(id=wxID_DIALOG1LBL_DBINFO,
              ...
              name=u'lbl_restore_db', parent=self,
              pos=wx.Point(25, 18), size=wx.Size(420, 175), style=wx.RAISED_BORDER)
        ...
        self.txt_licence1 = wx.TextCtrl(id=17, name=u'txt_licence1',
                     parent=self, pos=wx.Point(150, 110), size=wx.Size(270, 22),
                     style=0, value=u'')
        self.submit = wx.lib.buttons.GenButton(id=wxID_DIALOG1GENBUTTON1,
              label=gettext(u'register'), name='submit', parent=self,
              pos=wx.Point(100, 208), size=wx.Size(79, 26), style=0)
        self.submit.Bind(wx.EVT_BUTTON, self.OnsubmitButton,
              id=wxID_DIALOG1GENBUTTON1)

        self.close = wx.lib.buttons.GenButton(id=wxID_DIALOG1GENBUTTON2,
              label=gettext(u'close'), name='close', parent=self,
              pos=wx.Point(258, 208), size=wx.Size(79, 26), style=0)
        self.close.Bind(wx.EVT_BUTTON, self.OncloseButton,
              id=wxID_DIALOG1GENBUTTON2)
    ...
    def OnsubmitButton(self, event):
        ...
        try:
            ...
            pass
        if continueflag and len(ret)!=46:
            wx.MessageDialog(parent=None,
        ...
        if continueflag:
            ...
            if customer and customer==customer_code and self.get_mac().strip()==dmac:
                ...
                wx.MessageDialog(parent=None,
                    ...
                    style=wx.OK).ShowModal()
            if saveflag:
                zkeco,zktime,zkaccess=self.get_device_limit(licence.strip(),mac.strip(),customer.strip())
                try:
                    zkeco=authorize_fun.encrypt(str(zkeco).zfill(5))
                    zktime=authorize_fun.encrypt(str(zktime).zfill(5))
                    zkaccess=authorize_fun.encrypt(str(zkaccess).zfill(5))
                    format_mac=authorize_fun.encrypt(str(self.get_mac()).zfill(17))
                    #print 'zkeco:',zkeco,'zktime:',zktime,'zkaccess:',zkaccess,'mac:',format_mac,'\n'
                    if authorize_fun.Ini()==0:
                        if authorize_fun.CheckKey()==1:
                            authorize_fun.write_zkeco(zkeco)
                            authorize_fun.write_zktime(zktime)
                            authorize_fun.write_zkaccess(zkaccess)
                            authorize_fun.write_mac(format_mac)
                        else:
                            wx.MessageDialog(parent=None,
                        ...
                        wx.MessageDialog(parent=None,
                            message=u"%s"%gettext(u"insert_dog"),
                            caption=u"%s"%gettext(u"note"),
                            style=wx.OK).ShowModal()
                        return False
                except Exception,e:

...
def check_timeseg(start1, start1_vname, end1, end1_vname, start2, start2_vname, end2, end2_vname, start3, start3_vname, end3, end3_vname):
    ...
    elif start3 != zero and end3 != zero and start3 <= end2:#可以选择只使用前两个时间区间，最后一个留空。
...
class AccTimeSeg(CachingModel):
    ...
    def data_valid(self, sendtype):
        ...
        if self.id == 1:
            raise Exception(_(u'初始化的门禁时间段  %s 不能编辑') % self.timeseg_name)
        
        if tmp and tmp[0] != self:   #新增时不能重复。
            raise Exception(_(u'门禁时间段名称设置重复'))

        if AccTimeSeg.objects.count() == MAX_TIMESEG_COUNT:#默认255
            raise Exception(_(u'门禁时间段不能超过%s个') % MAX_TIMESEG_COUNT)
        ...
        check_timeseg(self.wednesday_start1, sun_start1_vname, self.wednesday_end1, wednes_end1_vname, self.wednesday_start2, wednes_start2_vname, self.wednesday_end2, wednes_end2_vname, self.wednesday_start3, wednes_start3_vname, self.wednesday_end3, wednes_end3_vname)
    ...
    def delete(self, *args, **kwargs):
        ...
        if init:
            ...
            if self.id == 1:
                raise Exception(_(u'初始化的门禁时间段  %s 不能删除') % self.timeseg_name)

            if self.acclevelset_set.all() or self.lockactive_set.all() or self.longopen_set.all() or self.accfirstopen_set.all():
                raise Exception(_(u'门禁时间段 %s 正在使用中,不能删除') % self.timeseg_name)
    ...
    class Admin(CachingModel.Admin):
        ...
        list_display = ("timeseg_name", "memo")

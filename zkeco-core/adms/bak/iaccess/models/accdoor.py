...
#OPENDOOR_CHOICES = (
#        (0, _(u'仅卡')), (1, _(u'卡加密码')), #(2, _(u'仅密码')), (3, (u'卡或密码')),
#)
        ...
        if door != cur_obj:
            try:
                ...
                if not p2.match(cur_obj.force_pwd):
                    door.force_pwd = decrypt(cur_obj.force_pwd)
                if not p2.match(cur_obj.supper_pwd):
                    door.supper_pwd = decrypt(cur_obj.supper_pwd)
...
class AccDoor(CachingModel):
        ...
        opendoor_type = models.SmallIntegerField(_(u'验证方式'), null=True, default=6, blank=False, editable=True, choices=OPENDOOR_CHOICES)#
        ...
        card_intervaltime = models.PositiveSmallIntegerField(_(u'刷卡间隔'), help_text=_(u'秒(范围:0-10)'), default=2, null=True, blank=False, editable=True)
        ...
        #video_linkageio = models.ForeignKey(Device, verbose_name=_(u'视频服务器'), related_name='videoserver_set', null=True, blank=True, editable=True)
...
#            super(AccDoor, self).delete()
        ...
        class Admin(CachingModel.Admin):
            ...
            menu_index = 100026
            list_display = ('door_no', 'door_name', 'lock_active', 'lock_delay', 'card_intervaltime', 'door_sensor_status', 'accfirstopen_set|detail_list_set', 'accmorecardset_set|detail_list_set')
...
class AccDevice(CachingModel):
    ...
    machine_type = models.IntegerField(_(u'设备类型'), null=True, blank=True, editable=False)#类似于DeviceName仅供内部使用-- C3_200 1， C3_400，C4-400 3 ， C3_100 4， C4_200 5 ， C4_400To_200 6 ，C3_400To_200 7 
...
def auto_add_door(dev_ins, door_count):
    for ndoor in range(door_count):
        AccDoor(device = dev_ins, door_no = (ndoor+1), door_name = '%s-%s' % (dev_ins.alias, str(ndoor+1))).save(force_insert=True)

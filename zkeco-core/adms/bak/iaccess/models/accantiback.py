...
from mysite.iclock.models.model_device import Device, DEVICE_ACCESS_CONTROL_PANEL, ACPANEL_1_DOOR, ACPANEL_2_DOOR, ACPANEL_4_DOOR
...
class AccAntiBack(CachingModel):
        ...
        def data_valid(self, sendtype):
            if not(self.one_mode or self.two_mode or self.three_mode or self.four_mode or self.five_mode):
        ...
        def getantibackoption(self):
            ...
            if self.one_mode and self.two_mode: #双门双向
                anti = 3
            elif self.one_mode:#单门双向，双门双向，四门单向
                anti = 1
            elif self.two_mode:
                anti = 2
        ...
        def get_details(self):
            ...
            elif self.device.acpanel_type == ACPANEL_2_DOOR and self.device.reader_count == 2:#两门单向
                door_name_2 = u'%s(%s)' % (names[1][1], names[1][0])
                if self.four_mode:
                    result.append(_(u"%(a)s 与 %(b)s 反潜") % {"a": door_name_1, "b": door_name_2})

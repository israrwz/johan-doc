...
def line_to_log(device, line, event=True):
    ...
    if settings.CHECK_DUPLICATE_LOG: #检查重复记录
        try:
            if Transaction.objects.exists(TTime=logtime, UserID=emp): 
                print u"该记录已经存在"
                return None
        except: pass
    ...
    if device.tz_adj <> None:
        logtime = logtime-datetime.timedelta(0, device.tz_adj * 60 * 60) + server_time_delta #UTC TIME, then server time

...
from mysite.att.att_calculate.att_rule import LoadAttRule,SaveAttRule
...
AttAbnomiteRptIDs=(1000,1007,1008,1009,1004,1001,1002,1005,1013,1012,1003)  #------- 参数页面控制选项
...
def NormalAttValue(Value, MinUnit,AttUnit, RemaindProc,WorkdayFlag=auHour,minsworkday=480):
    '''
    根据假类参数和请假时长来计算最后的有效请假时长
    
    Value：请假的有效时长（分钟）
    MinUnit：最小单位
    AttUnit：单位
    RemaindProc：是否按四舍五入计算
    WorkdayFlag：是否为工作日标识
    minsworkday：时段时间(分钟)
    '''
    global AttRule
...
def MainCalc(UserIDList,DeptIDList,d1,d2,isForce=0):
    '''
    从签卡记录中获取计算出每天的签到和签退时间
    UserIDList: 所选择的需要统计的用户ID列表
    d1,d2: 起止时间
    '''
    ...
    global isCalcing
    d1=trunc(d1)
    d2=trunc(d2+datetime.timedelta(days=1))
    if d1>d2: #---开始时间不能大于结束时间
        return -1
    if d2>trunc(datetime.datetime.now()+datetime.timedelta(days=1)):    #---结束时间不能大于现在时间
        return -3
    if not allowAction(d1,1,d2):    #---是否允许 ？?
        return -4
    if isCalcing==1:    #---同时只能有一个计算的线程
        return -5
    isCalcing=1
...
#考勤统计汇总/每日统计报表
def CalcReportItem_(request,reportType=0):
    '''
    汇总指定时间段内的考勤统计数据
    '''
...
#            Result['page_count']=page_count
        ...
        for uid in uids:#---------------------------------循环统计每个人员
            ...
            rows=cs.fetchall()
            if not rows:
                emp=Employee.objects.filter(id=uid)[0]
                for y in rmdAttday.keys():
                    rmdAttday[y]=''
                rmdAttday['userid']=uid
                rmdAttday['deptid']=emp.DeptID.name

...
function init_page(){
    ...
    $("#id_copy_right").click(function(){
        ...
        var version_logo = "";
        var oem_class = "";
        var att_version = "8.0.5";
        var acc_version = "5.0.1";
        ...
        var vbox=$("<div class='version_box'>"
                + "<div class='version_content_box"+oem_class+"'>"
                    + "<div class='version_details_box'><span>"
                        ...
                        + version
                        + gettext("本系统建议使用浏览器")
                        + ":</span><br />Internet Explorer 6.0+、Firefox 3.6+<br /><span>"
                        + gettext("显示器分辨率")
                        + ":</span><br />1024×768 "
                        + gettext("及以上像素")+"<br /><span>"
                        + gettext("软件运行环境")
                        + ":</span><br />WindowsXP/Windows2003/ Windows Vista/Windows7<br />MySQL("
                        + gettext("系统默认")
    ...
    $("#id_btn_option,#id_btn_changePW").click(function(){
        var a_this=this;
        var btn_values=[gettext("确定"),gettext("放弃")]
...
function OnCommError() {
    ...
    $.ajax({ 
        ...
        success:function(rtlog)
        {
            ...
            window.setTimeout('OnCommError()', 20000)//等*秒执行刷新函数
        }
    });
}

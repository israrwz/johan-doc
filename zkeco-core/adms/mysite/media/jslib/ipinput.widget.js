    ...
    keypress:function(obj,evt){
        ...
        if(key>=48 && key<=57)
        {
            ...
            if(parseInt(value,10)<255)
            {
                ...
                if(index==1)
                {
                    if(parseInt(value,10)>223 ||(value.length==3&&parseInt(value,10)==0))
                    {
                        if(evt.preventDefault)
                        {
                            evt.preventDefault();
    ...
    keydown:function(obj,evt){
        ...
        switch(key)
        {
            ...
            case 9:  //制表符
                if(index == 1)
                {
                    if(parseInt(value,10)==0 || parseInt(value,10)>223)
                    {
                        alert(gettext("请选择一个介于1到223之间的数值！"));
                        evt.returnValue=false;
                        obj.value="";

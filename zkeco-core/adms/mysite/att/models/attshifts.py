...
class attShifts(models.Model):
    ...
    ExceptionID=models.IntegerField(_(u'例外情况'),db_column='ExceptionID',  null=True,blank=True)  #----假类ID
    ...
    AttTime = models.IntegerField(_(u'时段时间(分钟)'), db_column='AttTime',null=True,blank=True)
    ...
    Exception=models.CharField(_(u'例外情况明细'), max_length=100,null=True,blank=True)   #---请假ID列表 外键到请假明细表
    ...
    def get_ExceptionID(self):
        ...
        try:
            if self.Exception:
                ...
                ex=AttException.objects.filter(UserID=self.UserID,pk__in=t)#----查询出所有请假记录
            else:
                ex=""
            if ex:
                val={}
    ...
    class Admin(CachingModel.Admin):
        ...
        list_display=('UserID.pk','UserID.DeptID.code','UserID.DeptID.name','UserID.PIN','UserID.EName','AttDate|format_date','SchId','ClockInTime|format_shorttime','ClockOutTime|format_shorttime',
                      ...
                      'WorkTime','get_ExceptionID','OverTime','WorkMins','SSpeDayNormal|format_int','SSpeDayWeekend|format_int',

...
#define GIL_LOCK(n) PyGILState_STATE _gilstate_##n = PyGILState_Ensure()	//线程状态控制
#define GIL_UNLOCK(n) PyGILState_Release(_gilstate_##n)

static int sockfd;	// 套接字ID
...
void server_run(const char* hostaddr, const int port)	//运行服务
{
  dnote("server_run ...");
  struct ev_loop* mainloop = ev_default_loop(0);	//libev 主循环

  ev_io accept_watcher;	//revent EV_READ, EV_WRITE			initialise an io watcher, then start it
  ev_io_init(&accept_watcher, ev_io_on_request, sockfd, EV_READ);	//watch for stdin to become readable  sockfd 为服务器的套接字
  ...
  ev_loop(mainloop, 0);			// now wait for events to arrive  em: ev_run (loop, 0);
...
bool server_init(const char* hostaddr, const int port)	// 服务器初始化 主要供python程序调用
{
  dnote("server_init ...");
  if((sockfd = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0)	// 打开套接字
    return false;

  struct sockaddr_in sockaddr;	//套接字地址对象
  sockaddr.sin_family = PF_INET;// 设置协议簇
  inet_pton(AF_INET, hostaddr, &sockaddr.sin_addr);//设置ip
  sockaddr.sin_port = htons(port);//设置端口

  /* Set SO_REUSEADDR t make the IP address available for reuse */	//设置套接字属性
  int optval = true;
  setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval));

  if(bind(sockfd, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) < 0)	//绑定套接字对象和套接字地址
    return false;

  if(listen(sockfd, LISTEN_BACKLOG) < 0)	//开始监听
    return false;

  DBG("Listening on %s:%d...", hostaddr, port);		//调试宏
  return true;
}
...
{
  dnote("ev_io_on_request ...");
  int client_fd;//定义客户端套接字
  struct sockaddr_in sockaddr;	//定义套接字地址
  socklen_t addrlen;

  addrlen = sizeof(struct sockaddr_in);
  client_fd = accept(watcher->fd, (struct sockaddr*)&sockaddr, &addrlen);	//绑定套接字地址到套接字对象 接受服务器套接字watcher->fd
  if(client_fd < 0) {
    DBG("Could not accept() client: errno %d", errno);
    return;
  }

  int flags = fcntl(client_fd, F_GETFL, 0);	//改变已打开的文件符号的性质		设置文件状态标记
  ...
  ev_io_init(&request->ev_watcher, &ev_io_on_read,
             client_fd, EV_READ);
  ev_io_start(mainloop, &request->ev_watcher);	//第二次
}
...
ev_io_on_read(struct ev_loop* mainloop, ev_io* watcher, const int events)
{
  ...
  Request_parse(request, read_buf, read_bytes);	//处理Request解析
...
{
  dprint("send_chunk",PyString_AS_STRING(request->current_chunk) + request->current_chunk_p);
